...
To deepen the understanding of the dynamics of \PAES, we also analyze its runtime on alternative benchmark functions such as $m$-LITZ (the $m$-objective “Leading Integers Trailing Zeros” benchmark) and $m$-FRITZ (the $m$-objective “First Run of Integers Trailing Zeros” benchmark) introduced in~\cite{Knowles2025}. For $x \in \{0,1\}^\ell$ denote by $\text{bin}(x)$ the integer represented by $x$ with respect to base 10. Then $m$-LITZ is defined as follows. 

\begin{definition}[Definition~2 in~\cite{Knowles2025}]
\label{def:LeadingIntegersTrailingZeros}
Suppose that the number $m$ of objectives is at least $8$ and a power of two, in particular $m = 2^p$ for $p \in \mathbb{N}$ with $p \geq 3$. Suppose that the problem size $n$ is divisible by $p$. Let $x=(x^1, \ldots , x^{n/p})$ where $x^j \in \{0,1\}^p$. Then, for $k \in [m-1]$, the $k$-th objective $\text{LITZ}_k(x)$ is defined as $\text{LITZ}_k(x) := \sum_{i=1}^{n/p} \prod_{j=1}^i I_k(\text{bin}(x^j))$ where $I_k:[2^p] \to \{0,1\}$, $I_k(\ell) = 1$ if $\ell=k$ and $0$ otherwise. The $m$-th objective is defined as $\text{LITZ}_m(x) := \sum_{i=1}^{n/p} \prod_{j=i}^{n/p} I_0(\text{bin}(x^j))$.
\end{definition}
...
Note that $2$-LITZ corresponds to $2$-LOTZ, but there are significant differences between $m$-LITZ and $m$-LOTZ, since in the former case, the trailing zeros only occur in one objective. Also note that in $m$-LITZ, all solutions $x$ have at most two different objectives distinct from zero. The following lemma is straightforward. 
...
\begin{proof}
Note that $m-2$ objectives out of $m-1$ are zero, since if $\text{binom}(x^1)=k$ then $f_1(x) = \ldots = f_{k-1}(x) = f_{k+1}(x) = \ldots = f_{m-1}(x)=0$. Further, the sum of the other two objectives cannot exceed $n/p$, since $I_k(\text{bin}(x^j)) \in \{0,1\}$, and if $I_k(\text{bin}(x^j)) =1$ then $I_\ell(\text{bin}(x^j))=0$ for $\ell \neq k$. So $V$ consists only of non-dominated fitness vectors. It suffices to show $V \subset f(\{0,1\}^n)$, proving the result. Let $v \in V$ and fix $i \in [m-1]$ with $v_i+v_m=n/p$. Consider $x=(x^1, \ldots ,x^p)$ with $\text{binom}(x^1) = \ldots = \text{binom}(x^{v_i}) = i$ and $\text{binom}(x^{v_i+1}) = \ldots = \text{binom}(x^m) = 0$. Then obviously $f(x)=v$. 
\end{proof}

Note that \PAES with one-bit mutation may be not able to optimize $m$-LITZ, as one can see if one considers $m=4$ and the current solution $x=1^{2 \lfloor{n/4}\rfloor}0^{n-2 \lfloor{n/4}\rfloor}$, while $A=\{y^{\ell}0^{n-2 \lfloor{n/4}\rfloor} \mid y \in \{00,01,10,11\}, \ell \in [\lfloor{n/4}\rfloor]\}$. Then flipping any bit in $x$ would result in a solution $y$ dominated by a solution $z$ from $A$, and hence, would not enter the archive $A$. So any mutant is discarded. However, \PAES with standard bit mutation shows that the whole Pareto front of $f := m$-LITZ can be found in polynomial time if $m=O(1)$. We will not use the hypervolume contribution, since $m-2$ objectives of each search point have value zero and hence, $\text{hv}(A_t)=0$ for each possible $A_t$. Instead, we define $N_t:=|\{f(x) \mid x \text{ is dominated by a point from } A_t\}|$. Note that $N_{t+1} \geq N_t$ and $1 \leq N_t \leq (m-1) \cdot (\sum_{i=1}^{n+1} i) = (m-1)(n+2)(n+1)/2$, since for a given $\ell \in \{0, \ldots , n\}$ there are $\ell+1$ different $\ell_1,\ell_2 \in \mathbb{N}_0$ with $\ell=\ell_1+\ell_2$.     

\begin{lemma}
\label{lem:LITZ-steps-current-solution-below}
Consider \PAES with standard bit mutation and archive size at least $n+1$ optimizing $m$-LITZ. Then the expected number of iterations until the current solution reaches the Pareto front is $O(mn^{3+\log(p)})$.
\end{lemma}
...
\begin{proof}
Note that each Pareto-optimal search point $y$ has the form $y=b^{\ell}(0^p)^{n/p-\ell}$ where $b \in \{0,1\}^\ell$. 
...
\begin{claim}
	\label{cl:reachfirsttwo-b-remaining-zero}
	Suppose that $f_m(s_{t_0}) =0^n$. After $O((n+m) \cdot 2^p)$ iterations in expectation, the current solution $s_t$ is non Pareto-optimal, or satisfies $f_m(s_t) = n/p-1$ and $f_k(s_t)=1$ which means that $s_t=b0^{n-p}$. 
\end{claim}

\begin{proofofclaim}
	As in the proof of the claims before, we may assume that always a Pareto optimal solution is created. At first we show that after $2n$ iterations, the probability that $(s_t)_i=1$ , unless a Pareto optimal solution $z$ is created with $z=a^\ell0^{n/p-\ell}$ with $a \in \{0,1\}^p$ and $\ell \in \{2, \ldots , n/p\}$. Then the first block in $s_t$ changes randomly, as only solutions of the form $c0^{n-p}$ for $c \in \{0,1\}^p$ are created, and also accepted. For a bit position $i \in [p]$, and $t \geq t_0$ denote by $q_i^{(t)}$ the probability that $(s_t)_i=1$. Then $q_i^{(t)}=(1-1/n) \cdot q_i^{{(t-1)}} + (1-q_i^{(t-1)})/n = 1/n + (1-2/n)q_i^{(t-1)}$ since flipping one specific bit happens with probability $1/n$. Note that this recursive equation has its equilibrium at $\hat{q}_i= 1/2$. By an easy induction, one sees that $q_i^{(t)} = 1/2 \cdot (1-(1-2/n)^t)$ (since $q_i^{(t_0)}=0$, and for $t>0$ we have $q_i^{(t)} = 1/n + (1-2/n)q_i^{(t-1)} = 1/n + (1-2/n) \cdot 1/2 \cdot (1-(1-2/n)^t) = 1/n + (1-2/n) \cdot 1/2 \cdot (1-(1-2/n)^t) = 1/n + (1-2/n)/2 - (1-2/n)^{t+1}/2 = 1/2-(1-2/n)^{t+1}/2 = 1/2(1-(1-2/n)^{t+1})$). So one sees that $q_i^{(t)} \in [0,1/2]$ and strictly monotone decreasing. For $t=t_0+n^2$ we have $q_i^{(t)} = 1/2(1-(1-2/n)^{n^2}) \geq 1/2(1-1/e^{2n}) \geq 1/2-1/(2e^{2n})$ due to $1+x \leq e^x$. Since we can consider all first $p$ bit positions as independent, and after $n$ iterations we see that $p_i^{(t)} \geq 1/2-1/(2e^{2n})$, we see that after $e(n^2+2^p) = e(n+m^2)$ iterations (provided that the second position does not coincide with the first) we created the desired $z=b0^{n-p}$.      
	...
	Denote an iteration $t$ as \emph{profitable} if a candidate solution $c_t$ is created with $f_m(c_t)>f_m(s_t)$. To create $s_t=0^n$, it is sufficient that the next $n/p - f_m(s_t) \leq 3$ iterations where the fitness of $s_t$ changes are profitable. We estimate the probability that an iteration where the fitness of $s_t$ changes is profitable. Let $q^*$ be the probability that one creates $0^{n/p}$ from $b \in \{0,1\}^{n/p}$ via standard bit mutation. Note that this also corresponds to the probability to create $b$ from $0^p$, and that $1/n^p \leq q^* \leq 1/n$. Then the probability that the fitness of $s_t$ changes is at most $2/n^p$ since it is required to either flip the first block $b$ from the left to $0^p$ (to increase $f_m(s_t)$ but decrease $f_k(s_t)$) or the last one from the left (to decrease $f_m(s_t)$ but increase $f_k(s_t)$). The probability 
	to change the rightmost block $b$ to $0^{n/p}$ while keeping the other blocks unchanged is at least $(1-1/n)^{n-p} \cdot p^* \geq p^*/e$ (to push $s_t$ towards $0^n$). Hence, an iteration where the fitness changes is profitable with probability at least $1/(2e) = \Omega(1)$. So the  next $j \leq 3$ iterations where the fitness changes are profitable with probability $\Omega(1)$. If this happens, $s_t=0^n$. The expected waiting time for a fitness change is $O(1/q^*)$. So in time $O(1/q^*)$, either $s_t=0^n$, or one of the first $n/p-f_m(s_t)$ iterations where the fitness changes is non-profitable (happening with probability $1-O(1)$). If the latter occurs, we repeat all the arguments from above, including those from Claim~\ref{cl:reachalmostzero}. The expected number of repetitions is $O(1)$, leading to an expected runtime of $O(n/p \cdot n^p + 1/p^*) = O(n/p \cdot n^p)$. This proves Claim~\ref{cl:reachfirsttwo-b-remaining-zero}. 
\end{proofofclaim}
