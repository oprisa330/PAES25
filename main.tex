...
Note that \PAES with one-bit mutation may be not able to optimize $m$-LITZ, as one can see with the following example. Consider $m=4$ and the current solution $x=1^{2 \lfloor{n/4}\rfloor}0^{n-2 \lfloor{n/4}\rfloor}$, while $A=\{y^{\ell}0^{n-2\ell} \mid y \in \{00,01,10,11\}, \ell \in [\lfloor{n/4}\rfloor]\}$. Then flipping any bit in $x$ would result in a solution $y$ dominated by a solution $z$ from $A$, and hence, would not enter the archive $A$. So any mutant is discarded.

However, \PAES with standard bit mutation shows that the whole Pareto front of $f := m$-LITZ can be found in polynomial time if $m=O(1)$. In difference to $m$-LOTZ, we will not use the hypervolume contribution in the analysis, since $m-2$ objectives of each search point have value zero and hence, $\text{hv}(A_t)=0$ for each possible $A_t$. Instead, we define $N_t:=|\{f(x) \mid x \text{ is weakly dominated by a search point from } A_t\}|$. Note that $N_{t+1} \geq N_t$ since $A_t$ preserves dominance \andre{should be sufficiently high!}, and $1 \leq N_t \leq (m-1) \cdot (\sum_{i=1}^{n/p+1} i) = (m-1)(n/p+2)(n/p+1)/2$, since for a given $\ell \in \{0, \ldots , n\}$ there are $\ell+1$ different $\ell_1,\ell_2 \in \mathbb{N}_0$ with $\ell=\ell_1+\ell_2$. \andre{maximum number of mutually incomparable solutions}

We will determine the expected time until \PAES covers the whole Pareto front of $m$-LITZ in a similar way as in the \LOTZ case above. A key difficulty is that the non-zero objective of the current solution $s_t$ among the first $m-1$ objectives can frequently change by modifying the first block, thereby creating a different binary number in that block. \andre{Add a roadmap here about the proof!}

\begin{theorem}
\label{lem:LITZ-Pareto-Front-Covering}
Consider \PAES with standard bit mutation and archive size at least $m(n/p+1)/2$ optimizing $m$-LITZ. Suppose that the current solution is not Pareto-optimal \andre{Bindestrich?}. Then the expected number of iterations until the current solution evolves a new Pareto optimum and adds it to the archive is $O(n^4p^{p-5}n^{(p^2 + p)/2})$. So the expected number of iterations to cover the Pareto front is $O(mn^5p^{p-6}n^{(p^2 + p)/2})$....
\end{theorem}

\begin{proof}
The proof consists of different cases. The first case is when the current solution $s_t$ is Pareto optimal. So let $s_t= c^j0^{n-j p} = c^j(0^p)^{n/p-j}$ for $c \in \{0,1\}^p$. The goal is to create a Pareto optimal $y$ which is not in the archive, say $y= b^{\ell}0^{n-\ell p} = b^{\ell}(0^p)^{n/p-\ell}$ where $b \in \{0,1\}^p$. To create such a $y$, one may start with $0^n$ as the current solution $s_t$ and then successively increase the number of $b$-blocks. During this process, however, the number of $b$-blocks may also decrease, all $b$-blocks may simultaneously change to $a$-blocks for some $a \in \{0,1\}^p$ with $a \neq b$, or the solution may leave the Pareto front.

The second case occurs when the current solution is not Pareto optimal. In this case, we first determine the expected time until $s_t$ becomes Pareto optimal. We then consider the random walk along the Pareto front until either a new Pareto-optimal solution is discovered and added to $A_t$ or the current solution leaves the Pareto front. 

We first consider the first case. Let $k$ be the binary value of $b$. We derive the desired expected runtime by proving a sequence of claims. A \emph{failure} occurs when $f_m$ changes by at least four in each iteration, or if $s_t$ becomes non-Pareto optimal. 

\begin{claim}
    \label{cl:reachalmostzero}
    Suppose that $s_{t_0}$ is Pareto optimal and satisfies $f_m(s_{t_0})=n/p-\ell$ for $\ell \in [n/p]$ in an iteration $t_0$. After $O(\ell n/p \cdot n^p)$ iterations in expectation, a failure occurs, or $f_k(s_t) \geq n/p-3$ holds which means that there exists $j \in [3]$ with $s_t=a^j 0^{n-jp}$. \andre{Could introduce different variables for $p$ above!}
\end{claim}
    
\begin{proofofclaim}
	Without loss of generality we may assume that $t_0=0$. Denote by $B_t$ the event that a failure occurs in iteration $t$ and let $T:=\inf\{t \geq 0 \mid B_t \text{ occurs or } f_k(s_t) \geq n/p-3\}$. For $j \in \{-4, \ldots , 4\}$ denote by $p_j$ the probability that $f_k(s_t)$ changes by $j$ if $s_t=a^\alpha 0^{n-p\alpha}$ for $\alpha > 3$ stays on the Pareto front. For $j \in [4]$ and $\alpha \leq n/p-j$ the probability to create $c_t=a^{\alpha+j} 0^{n-p(\alpha-j)}$ is the same as creating $c_t=a^{\alpha-j} 0^{n-p(\alpha+j)}$ by mutating $s_t$, since the probability to create $0^{jp}$ from $\alpha^p$ is the same as creating $\alpha^p$ from $0^{jp}$ via standard bit mutation. A different way to change $f_k(x)$ by $j \in [4]$ \andre{maybe reduce $j$ to three by considering only the changes? Possible since the expected number of fitness changes are $n^2/p^2$} is to create $d \in \{0,1\}^p$ from $a$ in the first $\alpha-j$ blocks, and then either mutate the remaining $a$-blocks to $0^p$ (to increase $f_k(x)$ by $j$), or mutate the remaining $a$-blocks to $c$, and also $j$ further $0$-blocks to $c$, while keeping the remaining bits unchanged. Note that in the case when $f_m(x)$ increases by $j$, $j \cdot \ones{a}$ specific bits have to be flipped, while in the case of decreasing $f_m(x)$ by $j$ one has to flip $jH(a,c)+j \ones{c} \geq j \ones{a}$ many bits (by the triangle inequality). If $\alpha > n/p-j$ we see that $p_{-j}=0$. So overall $p_j \geq p_{-j}$. \andre{zeige diese Möglichkeiten separat auf?} Further, $p_j \geq 1/(en^{jp})$ since all blocks which are distinct from the $j$ blocks immediately following $a^{\alpha}$ in $s_t$ remain unchanged with probability at least $(1-1/n)^{n-jp} \geq 1/e$. Let $X_t:=f_k(s_t)$ where $s_t$ denotes the current solution in iteration $t$. Note that $E(X_{t_0}^2) = (n/p-\ell)^2$, and $T=\inf\{t \geq 0 \mid B_t \text{ occurs or } X_t \geq n/p - 3\}$. We also see $\E(X_{t+1}-X_t \mid \bar{B}_0, \ldots \bar{B}_t,X_0, \ldots , X_t) = \sum_{j=-4}^4 j p_j \geq 0$, as $f_m(s_t)$ changes by at most $4$, and $\text{Var}(X_{t+1}-X_t \mid \bar{B}_0, \ldots \bar{B}_t, X_0, \ldots , X_t) \geq \E((X_{t+1}-X_t)^2 \mid \bar{B}_0, \ldots \bar{B}_t, X_0, \ldots , X_t) = \sum_{j=1}^4 j^2 p_j =:\delta$. Note that $\delta = \Omega(n^p)$. Therefore, $\E(T) \leq (n^2/p^2-E(X_{t_0}^2))/\delta = 2 \ell n/(\delta p)-\ell^2/\delta = O(\ell n/(\delta p)) = O(\ell n n^p/p)$ with Theorem~\ref{thm:unbiased-random-walk}, concluding the proof. 
\end{proofofclaim}
...
\begin{claim}
    \label{cl:reachzero}
    Suppose that $f_m(s_{t_0}) = n/p - \ell$ for some $\ell \in \{1,2\}$ at iteration $t_0$. Then, after $O(n \cdot n^p/p)$ iterations in expectation, a failure occurs or the current solution $s_t$ satisfies $f_m(s_t) = n/p$, that is, $s_t = 0^n$.
    \andre{Skizzen über Prozesse?}
\end{claim}
    
\begin{proofofclaim}
    As in the proof of the preceding claim, we may assume without loss of generality that $t_0 = 0$. Furthermore, we assume that no failure occurs. %Denote an iteration $t$ as \emph{profitable} if a candidate solution $c_t$ is created with $f_m(c_t)>f_m(s_t)$, and no failure occurs. 
    %To create $s_t=0^n$, it is sufficient that the next $n/p - f_m(s_t) \leq 2$ iterations where $f_k(c_t) \neq f_k(s_t)$ are profitable. 
    Suppose that $\ell=2$. Let $s_t=a^2 0^{n-2p}$ and denote by $\rho_a$ the probability to flip block $a$ to $0^p$ while not changing the other blocks. As in the proof in Claim~\ref{cl:reachalmostzero} before, we see that $p_j \geq p_{-j}$ for $j \leq \ell$ \andre{external lemma?}. Note that $p_1 = \Omega(\rho_a)$ as mutating the last block $a$ from the left to zero increases $f_k(s_t)$. Then, changing $f_m(s_t)$ requires to either flip the second block $a$ to $0^p$, or the third block $0^p$ to $a$, or the first and second block from $a$ to $d$ for $d \neq a$ and the third block from $0^p$ to $d$, while not changing the other blocks. Note that the first two events occur with probability $\rho_a$, whereas the last event occurs with probability at most $\rho_a$. So the probability that $f_m(c_t)>f_m(s_t)$ after such an iteration is at least $1/3$, since the latter event has probability at most $\rho_a$. So after $O(\rho_a)$ iterations, we have that $\ell \leq 1$, a failure occurs, or $n/p - 5 \leq f_k(s_t)<n/p-2$ \andre{mach oben eine zwei draus!}. If $f_k(s_t)<n/p-2$ we repeat the above arguments, possibly also those from Claim~\ref{cl:reachalmostzero}, to obtain a solution $s_t$ with $f_m(s_t) > n/p-2$. Note that the expected number of repetitions is $O(1)$.  
    
    If $\ell=1$, any solution of the form $d0^{n-j}$ is accepted where $d \in \{0,1\}^p$, meaning that if $c_t=d0^{n-j}$ is created from $s_t=a0^{n-j}$, then $s_{t+1}=c_t$. Let $T$ be the number of iterations until the desired $0^n$ is created, $n/p-5 \leq f_m(s_t) < n/p-2$, or a failure occurs. We show that $E(T)=O(n)$, and that the target string $0^n$ is generated within $T$ iterations with probability $\Omega(1)$. The probability that the second block matches the first block $d \neq 0^{n/p}$ in a single iteration is at most $1/n$, since this requires flipping at least one specific bit in the second block. So the probability that $f_m(s_t) \geq n/p-1$ within $8n+4m^2$ iterations is at least $(1-1/n)^{8n+4m^2} \geq (1-1/n)^{(8+4c^2)n} \geq 1/3^{8+4c^2} = \Omega(1)$ for $n$ sufficiently large. Moreover, this event $B$ is independent of events only concerning the first block, as each bit is flipped independently with probability $1/n$.  
    
    Suppose that $B$ occurs. For a bit position $i \in [p]$ denote by $q_i^{(t)}$ the probability that $(s_t)_i=1$. Then $q_i^{(t)}=(1-1/n) \cdot q_i^{(t-1)} + (1-q_i^{(t-1)})/n = 1/n + (1-2/n)q_i^{(t-1)}$ since flipping one specific bit happens with probability $1/n$. Note that this recursive equation has its equilibrium at $\hat{q}_i= 1/2$. By an easy induction, one sees that $q_i^{(t)} = 1/2 \cdot (1-(1-2/n)^t)$: Note that $q_i^{(0)}=0$, and for $t>0$ we have
    \begin{align*}
    	q_i^{(t)} &= 1/n + (1-2/n)q_i^{(t-1)} = 1/n + 1/2 \cdot (1-2/n) \cdot (1-(1-2/n)^t)\\
    	&= 1/n + (1-2/n)/2 - (1-2/n)^{t+1}/2 = 1/2-(1-2/n)^{t+1}/2 = 1/2(1-(1-2/n)^{t+1}).
    \end{align*}
    So one sees that $q_i^{(t)} \in [0,1/2]$ and $(q_i^{(t)})_{t \geq 0}$ is strictly monotone increasing. For $t=2n$ we have $q_i^{(t)} = 1/2(1-(1-2/n)^{2n}) \geq 1/2(1-1/e^4) \geq 0.49$ due to $1+x \leq e^x$ for all $x \geq 0$. \andre{we could extract this!}. Consequently, in each subsequent iteration, the probability to create $0^n$ is at least $4^{-p}(1-1/n)^{n-p} \geq m^{-2}/e$. By Markov's inequality, the probability that $0^n$ is created within $8n+4m^2$ iterations is $\Omega(1)$. So $B$ occurs and $0^n$ is created within $8n+4m^2$ iterations with probability at least $\Omega(1)$. If this does not happen, we repeat all the above arguments, including possibly those for the case $\ell=2$ and $n/p-5 \leq f_m(x) < n/p-2$. The number of repetitions is $O(1)$, concluding the proof of Claim~\ref{cl:reachzero}. 
\end{proofofclaim}
...
\begin{claim}
	...
	Suppose that $f_m(s_{t_0})=0^n$. After $O(n/p \cdot n^p)$ iterations in expectation, a failure occurs, or the current solution $t$ satisfies $f_m(s_t) = n/p-1$ and $f_k(s_t)=1$ which means that $s_t=b0^{n-p}$. 
\end{claim}

\begin{proofofclaim}
	Without loss of generality, assume that $t_0=0$ and that no failure occurs. Let $B$ denote the event that, within the next $8n+4m^2$ iterations, the second block of $s_t$ does not become identical to the first block, if the latter is distinct from zero. In particular, the second block remains $0^{n/p}$ when the first block is distinct from $0^{n/p}$. By the same argument as in the previous claim, we have $\Pr(B)=\Omega(1)$. Moreover, since all bits are flipped independently with probability $1/n$, the event $B$ is independent of the events concerning the first block. %Suppose that $B$ occurs. 
	
	As in the proof of Claim~\ref{cl:reachzero}, we wait for $2n$ iterations, after which the probability that $(s_t)_i=1$ is at least $1/4$, and then, we create $b$ with probability at least $4^{-p} = m^{-2}$. Note that none of the blocks from the second onward are changed with probability at least $(1-1/n)^{n-p} \geq 1/e$. So after  $4(2n + m^2) = O(n)$ iterations, with probability at least $\Omega(1)$, the desired $z=b0^{n-p}$ is created and $B$ occured. If this does not happen, we repeat all the above arguments, including possibly those from the previous claims \andre{which one?} while $n/p-4 \leq f_m(x) < n/p$. The number of repetitions is $O(1)$, concluding the proof of Claim~\ref{cl:reachfirst-b-remaining-zero}. 
\end{proofofclaim}
...
\begin{claim}
	...
	Suppose that $s_{t_0} =b^j0^{n-jp}$ at an iteration $t_0$ where $j \geq i$ for $i \in [\ell-1]$ and suppose that the expected number of iterations to evolve such an $s_{t_0}$ is $T_i$. Then, after $T_i(2+pn^{p-i}) + O(n^p)$ iterations in expectation, the current solution $s_t$ satisfies  $s_t =b^{j'}0^{n-j'p}$ for $j' \geq i+1$, or a failure occured \andre{Could add what $T_i$ is!}. Further, a search point $s_t =b^{i^*}0^{n-i^*p}$ for $i \leq i^*$ and $i \leq p+3$ is evolved after $O(n^2p^{i-3}\prod_{j=0}^{i-1}n^{p-j})$ iterations. Consequently, 
	$s_t =b^{i}0^{n-ip}$ for $i \geq p+3$ is evolved after $O(n^2p^{p-3} n^{(p^2+p)/2})$ iterations.
	
	\andre{Put in something for $T_i$!}
\end{claim}

\begin{proofofclaim}
The following can happen when the current solution is changed. A non Pareto-optimal solution is created, or $b^i0^{n-ip}$ is changed to $b^{i_1}0^{n-i_1p-p}$, or to $b^{i_2}0^{n-i_2p+p}$ for $i_1 < i < i_2$, or to a solution of the form $a^j0^{n-jp}$ for $j \in [n/p]$ and $a \in \{0,1\}^p$ with $a \neq b$ and $a \neq 0$. The probability of generating a solution of the form $a^j0^{n-jp}$ is at most $p/n \cdot  1/n^{j-1}$. This is because, in the first block, changing $b$ to $a$ requires flipping one of the $p$ bits in that block, which happens with probability at most $p/n$, while the corresponding bit flip must be also conducted in each of the remaining $j-1$ blocks, each with probability at most $1/n$. Then, conditioning on the event that the current solution changes, while a non-Pareto optimal individual is not created, the probability is at most $2 \rho_b + p/n \cdot 1/n^{j-1}$ that the current solution changes, where $\rho_b$ denotes the probability to change a block $b \in \{0,1\}^p$ to $0^p$ and vice versa, while not changing the other blocks. The probability is at least $\rho_b$ to create $b^{i+1}0^{n-ip-i}$ from $b^i0^{n-ip}$. So the expected number of changes of the fitness when $s_t=b^i0^{n-ip}$ for creating $s_{t+1}=b^{j}0^{n-jp}$ for $j>i$ is $(2\rho_b + p/n \cdot 1/n^{j-1})/\rho_b = 2+p/ (\rho_b \cdot n^j) \leq 2+p/ n^{j-p}$. If the fitness of $s_t$ changes, we pessimistically have to pass again through all claims previously mentioned in this lemma to obtain that after $T_i$ iterations, we have $s_t =b^j0^{n-jp}$ for $j \geq i$. Note that the expected time until a fitness change occurs when $s_t =b^j0^{n-jp}$ is $O(\min\{n^p,n^i/p\})$. So we see that $T_{i+1} = T_i(2+pn^{p-i}) + (2+pn^{p-i}) \cdot O(\min\{n^p,n^i/p\}) = T_i(2+pn^{p-i}) + O(n^p)$. Note that $T_1=O(n^2/p^2 \cdot n^p)$ and hence, by an easy induction on $i$, where $i \leq p$, $T_i=O(n^2/p^2 \cdot n^p \prod_{j=1}^{i-1} pn^{p-j}) = O(n^2p^{i-3} \prod_{j=0}^{i-1} n^{p-j})$. By pluggin $i=p+3$, we obtain $T_{p+3} = O(n^2p^{p-3} \prod_{j=0}^{p-1} n^{p-j}) = O(n^2p^{p-3} \prod_{j=1}^{p} n^j) = O(n^2p^{p-3} n^{(p^2+p)/2})$, proving the claim.  
...
\begin{claim}
	...
	Suppose that $s_{t_0} =b^{i}0^{n-ip}$ for $i \geq p+4$, and $\ell > p+3$. Then, after $O(n^2/p^2 \cdot n^p)$ advanced iterations in expectation, either the current solution is not Pareto optimal, or satisfies $s_t =b^\ell 0^{n-p\ell}$. So after $O(n^4p^{p-5} n^{(p^2+p)/2})$ iterations, the desired Pareto optimal solution is found, or the current solution is not Pareto optimal.
\end{claim}

\begin{proofofclaim}
Without loss of generality, assume that $t_0=0$. Call an advanced iteration a \emph{reset} if the current solution is Pareto optimal, but does not satisfy $s_t=b^i0^{n-ip}$ for $i \geq p+3$. A reset can only happen if the fitness of the current solution changes (happening with probability at least $2\rho_b + pn^{-p-3}$) and under this condition only with probability $pn^{-p-4} / (2\rho_b + pn^{-p-4}) = O(1/n^3)$. We show that $s_t$ coincides with $b^r0^{n-pr}$ for an $r \geq \ell-4$ after $O(n^2/p^2)$ advanced iterations in expectation where the fitness changed, a reset occured, or a non-Pareto optimal solution is created \andre{explain that we consider this advanced generations happening in a row. If we reach the zero state, then we do all the iterations from before again, and start again in a state at least zero! So the zero drift becomes still at least zero. Argue that if the number of ones in the block is at least two, then we do only jumps of at most one (otherwise: repetition!) and hence, fall back only $n/p$ times in epectation, since we can see our problem as a fair random walk!}. We use the zero drift theorem. Let $X_t=\max\{\kappa_t-p-2,0\}$ where $\kappa_t$ is the number of blocks $b$ occuring in the Pareto optimal current solution. For now, assume that the state, where $X_t=0$, is absorbing. We also see that $E(X_{t+1}-X_t \mid \ell - \kappa_t \geq 4) = \sum_{j=-4}^4 j p_j \geq 0$ since $p_{-j} \leq p_j$ for all $j \in [4]$, as $f_m$ changes by at most $4$ \andre{justify it here! Is this valid in general? Otherwise a failure occurs!}, and $\text{Var}(X_{t+1}-X_t \mid \bar{B}_{t_0}, \ldots \bar{B}_t, X_{t_0}, \ldots , X_t) \geq \E((X_{t+1}-X_t)^2 \mid \bar{B}_{t_0}, \ldots \bar{B}_t, X_{t_0}, \ldots , X_t) \geq \sum_{j=1}^4 j^2 p_j =:\delta = \Omega(1)$. Let $T':=\inf\{t \in \mathbb{N} \mid t \geq t_0 \text{ and } X_t \geq \ell-4\}$. Since we have to pass $\Omega(n^p)$ iterations in expectation until a change in fitness occurs, we obtain with Theorem~\ref{thm:unbiased-random-walk} that $E(T') = O(n^2/p^2 \cdot n^p + n/p \cdot n^3p^{p-4} n^{(p^2+p)/2}) = O(n^4p^{p-5} n^{(p^2+p)/2})$ \andre{justify this!}. To finally generate the desired Pareto optimal solution $b^{n/q}$ from $s_t=b^{n/p-4}(0^p)^4$, one has to change the block $0^p$ next to $b^{n/p-4+i}$ to $b$ in the next $i$-th generation when the fitness changes, for $i \in [4]$ happening with probability $\Omega(1)$. By repeating this at most four times, we create the desired Pareto optimal point. If this does not happen, we repeat all the arguments from above. The expected number of repetitions is $O(1)$, concluding the proof of this claim.       
\end{proofofclaim}

\begin{claim}
	\label{cl:small-way-back}
	Suppose that $s_{t_0} =b^{i}0^{n-ip}$ for $i > \ell$. Then, after $O(n^4p^{p-5} n^{(p^2+p)/2})$ iterations in expectation, a failure occurs or the current solution satisfies $s_t =b^\ell 0^{n-p\ell}$. \andre{guide the reader with sketches through the whole process!}
\end{claim}

\begin{proofofclaim}
	We show that, after $O((n/p)^2 n^p)$ generations in expectation, either $s_t =b^i0^{n-ip}$ for $i < \ell$, or $s_t =a^j0^{n-jp}$ for $a \neq b$, or a failure occurs. To this end, we use the zero drift theorem. For $j \in \{-f_k(s_t), \ldots , n/p-f_k(s_t)\}$ denote by $p_j \geq 1/(en^{jp})$ the probability that $f_k(s_t)$ increases by $j$.
	As in previous proofs, we may assume that $j \in \{-2,-1,0,1,2\}$. Note that all blocks in $s_t$ other than the $j$ blocks that should be changed remain unchanged with probability at least $(1-1/n)^{n-jp} \geq 1/e$. Let $X_t:=\min\{n/p-f_k(s_t),0\}$ where $s_t$ denotes the current solution in iteration $t$. Denote by $B_t$ the event that a failure occurs, $s_t=b^j 0^{n-pj}$ for $j < \ell$, or $s_t=a^i 0^{n-pi}$ for $i \in [n/p]$. Let $T=\inf\{t \geq 0 \mid B_t \text{ occurs or } X_t \geq n/p - \ell\}$. We also see $\E(X_{t+1}-X_t \mid \bar{B}_0, \ldots \bar{B}_t,X_0, \ldots , X_t) = \sum_{j=-2}^2 j p_j \geq 0$, as $f_m(s_t)$ changes by at most $2$, and $\text{Var}(X_{t+1}-X_t \mid \bar{B}_0, \ldots \bar{B}_t, X_0, \ldots , X_t) \geq \E((X_{t+1}-X_t)^2 \mid \bar{B}_0, \ldots \bar{B}_t, X_0, \ldots , X_t) = \sum_{j=1}^2 j^2 p_j =:\delta$. Note that $\delta = \Omega(n^p)$. Therefore, $\E(T) \leq (n^2/p^2-E(X_{t_0}^2))/\delta \leq n^2/(\delta p^2) = O(n^p n^2/p^2)$ with Theorem~\ref{thm:unbiased-random-walk}. If $B_t$ occurs for $1 \leq t \leq T$, then we apply all the claims before, to obtain that after further $O(n^4p^{p-5}n^{(p^2+p)/2})$ iterations in expectation, a failure occurs or a solution $c_t=b^i0^{n-ip}$ for $i \geq \ell$ is generated. Note that $b^\ell0^{n-\ell p}$ is created before $b^i0^{n-i p}$ with probability $1-o(1)$, concluding the proof of the claim by noting that at most $1+o(1)$ such periods for creating a solution $c_t=b^i0^{n-ip}$ for $i \geq \ell$ are necessary.   
\end{proofofclaim}

\begin{claim}
	\label{cl:climbing-to-front}
	Suppose that $s_t$ is not Pareto optimal. Then, after $O(n^4p^{p-5}n^{(p^2+p)/2})$ iterations in expectation, a new Pareto optimal point is added to $A_t$.   
\end{claim}

\begin{proofofclaim}
	If $s_t$ is not Pareto optimal, then $N_t < (m-1)(n/p+2)(n/p+1)/2$. To increase $N_t$ in one iteration, then it suffices to correct one block while not changing the other ones which happens with probability at least $1/(en^p)$. So the expected number of iterations until the Pareto front is reached is $O(m (n/p)^2n^p)$. Then, after further $O(n^4p^{p-5}n^{(p^2+p)/2})$ iterations a failure occurs which means that $s_t$ becomes again non-Pareto optimal, or is a new Pareto optimal point. However, creating a new Pareto optimal point and adding it into $A_t$ under the latter condition happens with probability at least $\Omega(1)$, concluding the proof of this claim.  
\end{proofofclaim}

So in total the expected number of generations to add a Pareto optimal solution into $A_t$ which has not added before is $O(n^4p^{p-5}n^{(p^2 + p)/2})$. Multiplying this with the size of the Pareto front (which is $O(mn/p)$) gives the desired runtime bound.
\end{proof}
