...
Consider \PAES with standard bit mutation and archive size at least $(m-1)(n/p+2)(n/p+1)/2$ optimizing $m$-LITZ. Then the expected number of iterations to cover the whole Pareto front is $O(m n^2/p^2....)$. 
\end{theorem}
...
\begin{claim}
	...
	Suppose that $s_{t_0} =b^{i}0^{n-ip}$ for $i > \ell$. Then, after $O(n^4p^{p-5} n^{(p^2+p)/2})$ iterations in expectation, a failure occurs or the current solution satisfies $s_t =b^\ell 0^{n-p\ell}$. \andre{guide the reader with skethes through the whole process!}
\end{claim}
