...
\begin{claim}
    \label{cl:reachzero}
    Suppose that $f_m(s_{t_0}) = n/p - \ell$ for some $\ell \in [2]$ at iteration $t_0$. Then, after $O(n \cdot n^p/p)$ iterations in expectation, a failure occurs or the current solution $s_t$ satisfies $f_m(s_t) = n/p$, that is, $s_t = 0^n$.
\end{claim}
    
\begin{proofofclaim}
    As in the proof of the claim before, we may assume that $t_0=0$. Denote an iteration $t$ as \emph{profitable} if a candidate solution $c_t$ is created with $f_m(c_t)>f_m(s_t)$, and no failure occurs. To create $s_t=0^n$, it is sufficient that the next $n/p - f_m(s_t) \leq 2$ iterations where $f_k(c_t) \neq f_k(s_t)$ are profitable. As in the proof in Claim~\ref{cl:reachalmostzero} before, we see that $p_j \geq p_{-j}$ for $j \leq \ell$. Note that $p_1 = \Omega(\rho_a)$ as mutating the last right block $a$ to zero increases $f_k(s_t)$. Suppose that $\ell=2$. Then, changing $f_m(s_t)$ requires to either flip the second block $a$ to $0^p$, or the third block $0^p$ to $a$, or the second block $a$ to $d$ for $d \neq a$, where $d$ is the first block after mutation and one can consider the first block as first mutated block. So the probability that such an iteration is profitable is at least $1/3$. If $\ell=1$, any solution of the form $a0^{n-j}$ is accepted. We show that in the next $O(n+m^2) = O(n)$ generations, with probability $\Omega(1)$ the desired $0^n$ is created, or the current solution $s_t$ becomes $s_t=d^j0^{n-pj}$ for a $j \in [3]$ and $d \in \{0,1\}^p$, or a failure occurs. At first we show that after $2n$ iterations, if all the latter scenarios do not happen, the probability that $(s_t)_i=1$ is at least $1/4$. Note that in this case the first block in $s_t$ changes randomly, as only solutions of the form $c0^{n-p}$ for $c \in \{0,1\}^p$ are created, and also accepted. For a bit position $i \in [p]$, and $t \geq t_0$ denote by $q_i^{(t)}$ the probability that $(s_t)_i=1$. Then $q_i^{(t)}=(1-1/n) \cdot q_i^{(t-1)} + (1-q_i^{(t-1)})/n = 1/n + (1-2/n)q_i^{(t-1)}$ since flipping one specific bit happens with probability $1/n$. Note that this recursive equation has its equilibrium at $\hat{q}_i= 1/2$. By an easy induction, one sees that $q_i^{(t)} = 1/2 \cdot (1-(1-2/n)^t)$: One sees $q_i^{(0)}=0$, and for $t>0$ we have
    \begin{align*}
    	q_i^{(t)} &= 1/n + (1-2/n)q_i^{(t-1)} = 1/n + 1/2 \cdot (1-2/n) \cdot (1-(1-2/n)^t)\\
    	&= 1/n + (1-2/n)/2 - (1-2/n)^{t+1}/2 = 1/2-(1-2/n)^{t+1}/2 = 1/2(1-(1-2/n)^{t+1}).
    \end{align*}
    So one sees that $q_i^{(t)} \in [0,1/2]$ and $(q_i^{(t)})_{t \geq t_0}$ is strictly monotone increasing. For $t=2n$ we have $q_i^{(t)} = 1/2(1-(1-2/n)^{2n}) \geq 1/2(1-1/e^4) \geq 0.49$ due to $1+x \leq e^x$ for all $x \geq 0$. \andre{we could extract this!}. Consequently, in each subsequent iteration, the probability to create $0^n$ is at least $4^{-p} = m^{-2}$. Note that the probability not to create any Pareto optimal search point with two blocks next to each other distinct from zero is $\Omega(1)$. If this does not happen, we repear all the above arguments under the consideration that no failure occurs \andre{also considering the remaining claims from above?}. The number of repetitions is $O(1)$, concluding the proof of Claim~\ref{cl:reachzero}. 
\end{proofofclaim}
