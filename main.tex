...
\begin{proofofclaim}
    As in the proof of the claim before, we may assume that $t_0=0$. Denote an iteration $t$ as \emph{profitable} if a candidate solution $c_t$ is created with $f_m(c_t)>f_m(s_t)$, and no failure occurs. To create $s_t=0^n$, it is sufficient that the next $n/p - f_m(s_t) \leq 2$ iterations where $f_k(c_t) \neq f_k(s_t)$ are profitable. As in the proof in Claim~\ref{cl:reachalmostzero} before, we see that $p_j \geq p_{-j}$ for $j \leq \ell$. Note that $p_1 = \Omega(\rho_a)$ as mutating the last right block $a$ to zero increases $f_k(s_t)$. Further, changing $f_m(s_t)$ requires to either flip such a block $a$ to $0^p$, or a block $0^p$ to $a$, or a block $a$ to $d$ for $d \neq a$, which has been created in a previous block. So the probability that such an iteration is profitable if $\ell = 2$ is at least $1/3$. If $\ell=1$, any solution of the form $a0^{n-j}$ may be accepted. This costs also $n+m$ iterations to make the first component random, and then create zero without creating a solution coinciding in the first two components.    
    
    but then also  
    
    
     We estimate the probability that such an iteration is profitable. Denote by $p_a$ the probability to mutate $a$ into $0^p$.     
    
    The probability that $f_k(c_t) \neq f_k(s_t)$ without happening a failure is at most $2\rho_a$ since it is required to either flip the first block $a$ from the right to $0^p$ (to increase $f_m(s_t)$ but decrease $f_k(s_t)$) or the first block $0^p$ from the left (to decrease $f_m(s_t)$ but increase $f_k(s_t)$). The probability 
